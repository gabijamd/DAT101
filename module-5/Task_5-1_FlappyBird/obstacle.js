...
export class TObstacle{
  ...
  constructor(aSpcvs, aSPI){
    ...
    this.#spDown = new TSprite(aSpcvs, aSPI.obstacle, x, topWithGap);
    this.#spDown.index = 2;
    this.#spUp = new TSprite(aSpcvs, aSPI.obstacle, x, top);
    this.#spUp.index = 3;
  }
  ...
  animate(aCanvasWidth){
    ...
    if(this.#spDown.x < -this.#spDown.width) {
      this.#spDown.x = aCanvasWidth;
    }
    if(this.#spUp.x < -this.#spUp.width) {
      this.#spUp.x = aCanvasWidth;
    }
  }
